...
# Alternar para a planilha original (pasta de downloads fabricação própria)
pyautogui.hotkey('alt', 'tab')  # Alternar para a janela anterior
time.sleep(2)  # Aguarda 2 segundos para garantir que a janela foi alternada

# Selecionar o intervalo de C9 até a célula final na coluna C
if celula_final and celula_final[1:].isdigit():  # Verifica se a entrada do usuário é válida
    pyautogui.hotkey('ctrl', 'g')  # Abrir a caixa "Ir Para"
    time.sleep(2)  # Aguarda para garantir que a caixa seja aberta
    pyautogui.write(f'C9:C{celula_final[1:]}')  # Digitar o intervalo de C9 até a célula final
    time.sleep(1.5)  # Pequena pausa antes de pressionar Enter
    pyautogui.press('enter')  # Confirmar a seleção do intervalo
    time.sleep(2)  # Garantir que o intervalo foi selecionado

    # Copiar o intervalo selecionado para a área de transferência
    pyautogui.hotkey('ctrl', 'c')  # Atalho para copiar
    time.sleep(2)  # Garantir que a cópia foi concluída

# Alternar para a planilha "Base consulta usinagem"
pyautogui.hotkey('alt', 'tab')  # Alternar para a janela anterior
time.sleep(2)  # Aguarda 2 segundos para garantir que a janela foi alternada
...
pyautogui.hotkey('ctrl', 'g')  # Abrir a caixa "Ir Para"
time.sleep(2)  # Aguarda 2 segundos para garantir que a caixa seja aberta
pyautogui.write('B3')  # Digitar a célula B3
time.sleep(1)  # Pequena pausa antes de pressionar Enter
pyautogui.press('enter')  # Confirmar
time.sleep(2)  # Garantir que a célula foi selecionada
